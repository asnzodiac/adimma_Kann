...
MANGLISH_KEYWORDS = {
    ...
    "aadhyam", "pinne", "sheri", "adipoli", "adipoli", "pwoli", "pwoli",
    "nannayit", "nannayirunnu", "arike", "arikil", "kashtu", "kshtam",
    "enthelum", "enikku", "thante", "thante", "eppol", "eppo", "enga",
...
def maybe_handle_weather(text: str) -> Optional[str]:
    ...
    pattern = re.search(
        r"\b(?:weather|temperature|temp|climate|rain|sunny|humid)\b.*?(?:in|at|for)?\s*([A-Za-z\s]{2,30})",
        text,
        re.IGNORECASE,
    )
    if pattern:
...
def check_sleep_wake(text: str, chat_id: int) -> Optional[str]:
    ...
    if sleeping_chats[chat_id]:
        if any(wake in clean for wake in WAKE_TRIGGERS):
            ...
            return (
                "👁 Aye, I'm back, sir! What chaos are we getting into now? 😏"
            )
        # Still sleeping — no response
        return ""  # empty string = silently ignore
...
def handle_start(chat_id: int, user: dict, chat_type: str) -> None:
    ...
    # Notify owner if it's a new private user (not the owner themselves)
...
def handle_clear(chat_id: int) -> None:
    ...
    send_text(
        ...
        "_(I still remember your personality though — unfortunately.)_ 😄"
    )
...
def process_voice_message(chat_id: int, voice: dict, message_id: int) -> None:
    ...
    try:
        ...
        # Detect language (from transcript text, override STT detection if needed)
...
def handle_update(update: dict) -> None:
    ...
    if text:
        ...
        if sleep_wake_response is not None:
            if sleep_wake_response:  # Non-empty = send response
                send_text(chat_id, sleep_wake_response)
            return  # Either sleeping (ignore) or just handled sleep/wake
    ...
    if "voice" in message or "audio" in message:
        ...
        process_photo_message(chat_id, photos, caption, message_id)
        ...
        # Caption only (no known media type handled above)
